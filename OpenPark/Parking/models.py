...
def upload_parking_picture(instance, filename):
    return "static/pictures/{id}_{host_to}/{filename}".format(
        host_to=instance.name, filename=filename, id=instance.id
    )
...
def upload_parking_picture(instance, filename):
    return "static/pictures/{id}_{host_to}/{filename}".format(
        host_to=instance.code, filename=filename, id=instance.id
    )
...
class Parking(models.Model):
    ...
    address = models.CharField(
        max_length=255, null=True, blank=True, default="Kathmandu"
    )
